var eventContests = function (eventContests) {

    var eventID             = null,
        allStages           = null,
        newContest          = null,
        allJudgesOptions    = null,
    ...
    /**
     * Submit Creating Contest
     * @private
     */
    var createContest_ = function (event) {
        ...
        var ajaxData = {
            ...
            data: formData,
            beforeSend: function(){
                vp.form.addLoadingClass(newContest);
            },
            success: function(response) {
                response = JSON.parse(response);
                vp.form.removeLoadingClass(newContest);

                if (parseInt(response.code) === 130) {
            ...
            error: function(response) {
                vp.core.log('ajax error occur on creating contest','error',corePrefix, response);
                vp.form.removeLoadingClass(newContest);
            }
        };
     ...
     * @param id - contest ID
     */
    eventContests.edit = function (id) {
     ...
     * @param id - contest ID
     */
    eventContests.delete = function (id) {
        vp.notification.notify({
            type: 'confirm',
            message:
    ...
    var prepareNewContest_ = function () {
        ...
        if (!newContest || !judges || !formula) {
            vp.core.log('Could not catch `#newContest`, `#newContestJudges`, `#newContestFormula`', 'error', corePrefix);
            return;
        }

        newContest = newContest.querySelector('.block');
        newContest.addEventListener('submit', createContest_);
